...
strokePath ::
           (MonadIO m) => CanvasRenderingContext2D -> Maybe DOMPath -> m ()
...
isPointInPathPath ::
                  (MonadIO m) =>
                    CanvasRenderingContext2D ->
                      Maybe DOMPath -> Float -> Float -> CanvasWindingRule -> m Bool
...
isPointInStrokePath ::
                    (MonadIO m) =>
                      CanvasRenderingContext2D ->
                        Maybe DOMPath -> Float -> Float -> m Bool
...
putImageData self imagedata dx dy
  = liftIO
      (js_putImageData (unCanvasRenderingContext2D self)
         (maybe jsNull unImageData imagedata)
         dx
         dy)
...
drawFocusIfNeededPath ::
                      (MonadIO m, IsElement element) =>
                        CanvasRenderingContext2D -> Maybe DOMPath -> Maybe element -> m ()
