  ...
  = liftIO
      (fromJSString <$>
         (js_xhrResponseSource (unInternals self)
            (maybe jsNull unXMLHttpRequest xhr)))
...
markerRangeForNode ::
                   (MonadIO m, IsNode node, ToJSString markerType) =>
                     Internals -> Maybe node -> markerType -> Word -> m (Maybe DOMRange)
...
rangeFromLocationAndLength ::
                           (MonadIO m, IsElement scope) =>
                             Internals -> Maybe scope -> Int -> Int -> m (Maybe DOMRange)
...
locationFromRange ::
                  (MonadIO m, IsElement scope) =>
                    Internals -> Maybe scope -> Maybe DOMRange -> m Word
...
foreign import javascript unsafe
        ...
        js_rangeForDictionaryLookupAtLocation ::
        JSRef Internals -> Int -> Int -> IO (JSRef DOMRange)
...
rangeForDictionaryLookupAtLocation ::
                                   (MonadIO m) => Internals -> Int -> Int -> m (Maybe DOMRange)
...
openDummyInspectorFrontend ::
                           (MonadIO m, ToJSString url) =>
                             Internals -> url -> m (Maybe DOMWindow)
...
closestTimeToTimeRanges self time ranges
  = liftIO
      (js_closestTimeToTimeRanges (unInternals self) time
         (maybe jsNull unTimeRanges ranges))
